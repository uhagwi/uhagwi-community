...
**Phase 3에서 사용자가 *직접 골랐거나 강하게 동의한* 업무**를 자동화 후보로 정제.
Phase 2의 모든 업무를 자동화로 옮기는 게 아니라, 사용자 욕구·금기·품질 기준에 *부합하는 것만*.
동의한 만큼만 담되 **최소 1개, 최대 8개** — 없는 욕구를 억지로 채우지도, 빈 배열로 두지도 말 것.
...
- **auto_candidates 1~8개** (Phase 3에서 사용자 동의 받은 것만, 빈 항목 X). 동의가 약해도 가장 가능성 높은 후보를 **최소 1개는 반드시** 포함
...
- auto_candidates 빈 배열(0개) 또는 9개 이상`;
